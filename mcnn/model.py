...
def data_generator(dataset, config, shuffle=True, augment=False, augmentation=None,
    ...
    # if the model type is MASK_RCNN, then it will use the pyramid method to generate anchor
    ...
    # if the model type is P_MASK_RCNN, then it will use the probability-based method to generate anchor
    elif config.MODEL_TYPE == "P_MASK_RCNN":
        anchors = utils.generate_probability_anchors(config.RPN_ANCHOR_SCALES,
                                                     config.RPN_ANCHOR_RATIOS,
                                                     sample_points,
                                                     config.DOWN_SAMPLE_STRIDE,
                                                     config.IMAGE_SHAPE)
    # sample_points = np.broadcast_to(sample_points, (config.BATCH_SIZE,) + sample_points.shape)
...
class MaskRCNN():
    ...
    def __init__(self, mode, config, model_dir, sample_points):
        """
        mode: Either "training" or "inference"
        config: A Sub-class of the Config class
        sample_points: the pixels that will be used to generate anchor on the original image
    ...
    def build(self, mode, config):
        ...
        # the P6 is not used in RPN, as it is usually used to detect large objects
        ...
        # out = KL.Lambda(lambda t: t, name="out")(p5_select_index)
        # out = sampling_module.sampling_graph(sample_ids, 8, config.IMAGE_SHAPE, config.BATCH_SIZE, name="sample_out")

        # Anchors
        if mode == "training":
            anchors = self.get_anchors(config.IMAGE_SHAPE)
            # box = utils.denorm_boxes(anchors, [512, 512])
            # ids = np.random.randint(0, len(box), 1000)
            # # ids = np.arange(0, len(box))
            # print(len(box))
            # print(box[ids])
            # image = plt.imread("../images/01123.jpg")
            # visualize.draw_boxes(image, refined_boxes=box[ids], title="generate box")
            # plt.show()
            # exit()
        ...
        # p_rpn_out = p_rpn([up_scale_p2, p2_select_index])
        # out = KL.Lambda(lambda t: t+1, name="sample_out")(p_rpn_out[0])
        # Loop through pyramid layers
        layer_outputs = []  # list of lists
        # if the model type is MASK_RCNN, the original RPN will be choosen to build the rpn graph
        if config.MODEL_TYPE == "MASK_RCNN":
        ...
        # out = KL.Lambda(lambda t: t+1, name="sample_out")(rpn_class_logits)
        # Generate proposals
    ...
    def get_anchors(self, image_shape):
        ...
        if not tuple(image_shape) in self._anchor_cache:
            ...
            elif self.config.MODEL_TYPE == "P_MASK_RCNN":
                if self.sample_points is None:
                    raise Exception("sample_points must be provided in P_MASK_RCNN model")
                a = utils.generate_probability_anchors(
                    self.config.RPN_ANCHOR_SCALES,
                    self.config.RPN_ANCHOR_RATIOS,
                    self.sample_points,
                    self.config.DOWN_SAMPLE_STRIDE,
    ...
    def run_graph_on_train_mode(self, input_data, outputs):
        """Runs a sub-set of the computation graph that computes the given
        outputs on training mode.

        input_data: all the input layer for the graph on training mode
...
if __name__ == "__main__":
    ...
    # MODEL_PATH = "../logs/carotid20190826T1810/mask_rcnn_carotid_0080_.h5"
    import pulmonary_embolism
    from mrcnn import visualize

    config = pulmonary_embolism.PulmonaryConfig()
    ...
    DATA_DIR = "G:\\longkun\\dataset\\pulmonary_embolism"
    dataset = pulmonary_embolism.PulmonaryEmbolismDataset()
    dataset.load_pulmonary_embolism(DATA_DIR, "test")
    dataset.prepare()
    MODEL_PATH = "G:\longkun\pythonWorkspace\Mask_RCNN\logs\\a_pulmonary20190710T1738\mask_rcnn_pulmonary_0080.h5"
    model = MaskRCNN(mode="training", config=config, model_dir=MODEL_DIR, sample_points=dataset.sample_points)
    ...
    [normalized_images, image_meta, rpn_match, rpn_bbox, gt_class_ids, gt_boxes, gt_masks, sample_points], _ = next(g)
    # print(sample_points)
    mrcnn = model.run_graph_on_train_mode(
        [normalized_images, image_meta, rpn_match, rpn_bbox, gt_class_ids, gt_boxes, gt_masks, sample_points], [
            ("proposals", model.keras_model.get_layer("ROI").output),
            ("probs", model.keras_model.get_layer("mrcnn_class").output),
            ("deltas", model.keras_model.get_layer("mrcnn_bbox").output),
            ("masks", model.keras_model.get_layer("mrcnn_mask").output),
            # ("out", model.keras_model.get_layer("out").output)
        ])
    # print(mrcnn["out"])
    # print(mrcnn["out"].shape)
